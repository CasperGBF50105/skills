...
Создаёт два файла:
  - Учет_вагонов_с_тестовыми_данными.xlsx — с примерами (3 вагона, 5 платежей)
  - Учет_вагонов_шаблон.xlsx              — пустой, готовый для реальных данных
    (одна пустая строка с формулами в каждой таблице, чтобы при добавлении
    новых строк Excel сам протягивал формулы вниз по таблице)

Совместимость формул: используются только функции, доступные в Excel
2007+ (SUMIFS, COUNTIF, LOOKUP, SUMPRODUCT, IF, IFERROR, AND, SUM, COUNTA).
XLOOKUP (Excel 365/2021+) и MAXIFS (Excel 2016+) сознательно не
используются — в более старых версиях Excel/WPS они показывают #ИМЯ?
(#NAME?).
"""
...
def build_workbook(output, with_samples):
    """with_samples=True — книга с тестовыми данными;
    with_samples=False — пустой шаблон (по одной пустой строке с формулами
    в каждой таблице, кроме Курсы, где строк вовсе нет — это справочник,
    который пользователь заполняет своими реальными курсами)."""
    ...
    # 2. Курсы
    # ============================================================
    # Важно: строки этой таблицы должны идти по возрастанию даты — формулы
    # LOOKUP на листах Вагоны и Платежи ищут ближайший курс на дату
    # ИЛИ РАНЬШЕ и требуют, чтобы даты были отсортированы по возрастанию.
    ...
    for r in range(2, ws.max_row + 1):
        ws.cell(row=r, column=8).value = (
            "=LOOKUP([@[Дата фиксации цены]],tblКурсы[Дата],tblКурсы[Курс])"
        )
        ...
        ws.cell(row=r, column=24).value = (
            '=IFERROR(SUMPRODUCT(MAX((tblПлатежи[ID вагона]=[@[ID вагона]])*'
            '(tblПлатежи[Статус]="факт")*(tblПлатежи[Дата платежа]))),"")'
        )
    ...
    for r in range(2, ws.max_row + 1):
        ws.cell(row=r, column=5).value = (
            "=LOOKUP([@[Дата платежа]],tblКурсы[Дата],tblКурсы[Курс])"
        )
        ...
        ws.cell(row=r, column=10).value = (
            "=COUNTIF(INDEX(tblПлатежи[ID вагона],1):[@[ID вагона]],[@[ID вагона]])"
        )
    ...
    if with_samples:
        ...
        rows_plan = [[None] * len(headers_plan)]
...
if __name__ == "__main__":
    build_workbook("Учет_вагонов_с_тестовыми_данными.xlsx", with_samples=True)
    build_workbook("Учет_вагонов_шаблон.xlsx", with_samples=False)
